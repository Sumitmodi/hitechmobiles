...
function xlsx(file) { 
   ...
   if (typeof file === 'string') { // Load
		...
		while (i--) {
			...
			while (--j) { // Don't process j === 0, because s[0] is the text before the first row element
				...
				if (getAttr(s[j], 'customHeight') === '1') {
					height = +getAttr(s[j].substring(s[j].indexOf('customHeight') + 13), 'ht');
					if (isNaN(height)) {
						height = +getAttr(s[j], 'ht');
					}
					row.height = height;
				}
				// GrapeCity End
		...
		while (w--) { 
			...
			if (worksheet.table) {
				i = -1; l = data[0].cells.length;
				/* GrapeCity: Fix issue exporting empty sheet 'dimension ref' property incorrect.*/
				t = data[0].cells.length > 0 ? ':' + numAlpha(data[0].cells.length - 1) + data.length : '';
				s = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><table xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" id="' + id
					+ '" name="Table' + id + '" displayName="Table' + id + '" ref="A1' + t + '" totalsRowShown="0"><autoFilter ref="A1' + t + '"/><tableColumns count="' + data[0].cells.length + '">';
				while (++i < l) { 
