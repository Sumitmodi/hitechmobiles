...
//		get_instance()->session->set_flashdata("__messages", serialize($this->_messages));
    ...
    private function get_template_output($output) {
        ...
        if (function_exists("get_instance") && class_exists("CI_Controller")) {
            ...
            $navActive = $ci->load->get_current_nav();
          //  if (!$ci->session->userdata('logged_exotic')) {
              /*  $ci->load->model('Admin_Model');
                $selectArr = array('sn', 'name');
                $destinationsArr = $ci->Admin_Model->getCommonTable('', 'destinations', 'name', 'asc', $selectArr);
                $data['destinationMenuArr'] = $destinationsArr;*/
