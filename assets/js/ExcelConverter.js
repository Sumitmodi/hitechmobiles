        ...
        var ExcelConverter = (function () {
            ...
            */
            ExcelConverter.import = function (file, flex, convertOption, moreSheets) {
                ...
                // wijmo.importData  = this.moreSheets;
                // return;
            };
            ...
            */
                ...
                for (; sheetIndex < sheetCount; sheetIndex++) {
                    ...
                    for (; r < currentSheet.rows.length; r++) {
                        ...
                        if (mainImportData) {
                            ...
                            // this.moreSheets = mainImportData;
                            wijmo.importData  = mainImportData;
                            return;
                        };
                        ...
                        if (row && !!row.height && !isNaN(row.height)) {
                            flex.rows[r].height = row.height * 96 / 72;
                        }
                        ...
                        for (c = 0; c < columnCount; c++) {
                            if (!row) {
                                ...
                                //flex.dataSource._data.push(currentIncludeRowHeader ? r - 1 : r, c, formula && moreSheets ? formula : this._getItemValue(item));
            ...
            ExcelConverter._parseFlexGridRowToSheetRow = function (flex, row, rowIndex, columnSettings, includeColumnHeader, isGroupRow, groupLevel) {
                ...
                isCommonRow = row.constructor === wijmo.grid.Row;
                ...
                for (var ci = 0; ci < flex.columnHeaders.columns.length; ci++) {
                    ...
                    if (sheetInfo) {
                        ...
                        mergedCells = flex.getMergedRange(flex.cells, rowIndex, ci);
                    ...
                    if (isCommonRow || isGroupRow) {
                        ...
                        format = columnSetting.style.format ? columnSetting.style.format : wijmo.isDate(unformattedVal) ? 'M/d/yyyy' : !wijmo.isNumber(unformattedVal) || wijmo.isInt(unformattedVal) ? 'General' : this._formatMap['n'];
                    } else {
            ...
            ExcelConverter._parseCellStyle = function (cellStyle) {
                return {
                    font: {
                        ...
                        size: cellStyle && cellStyle.fontSize ? cellStyle.fontSize.substring(0, cellStyle.fontSize.indexOf('px')) : '12',
            ...
            ExcelConverter._parseExcelFormat = function (item) {
                ...
                var formatCode = item.style && item.style.formatCode ? item.style.formatCode : '', format = '', lastDotIndex;
                if (wijmo.isDate(item.value)) {
                    if (!formatCode || formatCode === 'General') {
                        return 'M/d/yyyy';
                    }
                    format = formatCode.replace(/[\/\s\-,]m+|m+[\/\s\-,]/, function (str) {
